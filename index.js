...
app.get('/olx/terrenos', (req, res) => {
    ...
    axios.get(page).then(response => {
        ...
        if (nextpage != undefined) {
            retorno.proxima = 'http://localhost:3000/olx/terrenos?page=' + nextpage.split('?o=')[1] + (q != undefined? '&q=' + q : '')
            retorno.anterior = 'http://localhost:3000/olx/terrenos?page=' + Number(nextpage.split('?o=')[1] - 1) + (q != undefined? '&q=' + q : '')
        }
...
app.get('/olx/carros', (req, res) => {
    ...
    axios.get(page).then(response => {
        ...
        if (nextpage != undefined) {
            retorno.proxima = 'http://localhost:3000/olx/carros?page=' + nextpage.split('?o=')[1] + (q != undefined? '&q=' + q : '')
            retorno.anterior = 'http://localhost:3000/olx/carros?page=' + Number(nextpage.split('?o=')[1] - 1) + (q != undefined? '&q=' + q : '')
        }
